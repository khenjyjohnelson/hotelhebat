...
class C_tabel13 extends Omnitags
{
	...
	public function admin()
	{
		...
		$data1 = array(
			'title' => $this->v3_title['tabel13_alias'],
			'konten' => $this->v3['tabel13'],
			'dekor' => $this->tl12->dekor('tabel13')->result(),
	...
	public function tambah()
	{
		...
		if (!$this->upload->do_upload($this->v_input['tabel13_field4_input'])) {
			...
			$this->session->set_flashdata($this->views['flash2'], $this->flash_msg2['tabel13_field4_alias']);
		...
		if ($simpan) {
			$this->session->set_flashdata($this->views['flash1'], $this->flash1_msg_1['tabel13_alias']);
			$this->session->set_flashdata('toast', $this->views['flash1_func1']);
		} else {
			$this->session->set_flashdata($this->views['flash1'], $this->flash1_msg_2['tabel13_alias']);
			$this->session->set_flashdata('toast', $this->views['flash1_func1']);
		}
	...
	public function update() //update tidak diperlukan di sini
	{
		...
		if ($update) {
			$this->session->set_flashdata($this->views['flash1'], $this->flash1_msg_3['tabel13_alias']);
			$this->session->set_flashdata('toast', $this->views['flash1_func1']);
		} else {
			$this->session->set_flashdata($this->views['flash1'], $this->flash1_msg_4['tabel13_alias']);
			$this->session->set_flashdata('toast', $this->views['flash1_func1']);
			redirect($_SERVER['HTTP_REFERER']);
		}
	...
	public function hapus($tabel13_field1 = null)
	{
		...
		if ($hapus) {
			$this->session->set_flashdata($this->views['flash1'], $this->flash1_msg_5['tabel13_alias']);
			$this->session->set_flashdata('toast', $this->views['flash1_func1']);
		} else {
			$this->session->set_flashdata($this->views['flash1'], $this->flash1_msg_6['tabel13_alias']);
			$this->session->set_flashdata('toast', $this->views['flash1_func1']);
		}
	...
	public function laporan()
	{
		...
		$data1 = array(
			'title' => $this->v4_title['tabel13_alias'],
			'dekor' => $this->tl13->dekor('tabel13')->result(),
			'tbl13' => $this->tl13->ambildata()->result()
		);
